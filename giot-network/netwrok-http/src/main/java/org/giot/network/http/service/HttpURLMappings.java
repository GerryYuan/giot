 ...
 */
public class HttpURLMappings implements URLMappings {
    ...
    public List<String> mappings() {
        return urls().stream().map(s -> version().toString().concat(s)).collect(Collectors.toList());
    }

    @Override
    public String mapping(final MsgVersion version, final String url) {
        return "/".concat(version().toString()).concat(url);
    }
